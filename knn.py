...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i][j] = torch.sqrt(torch.sum((torch.square(x_train[i].flatten()-x_test[j].flatten()))))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    test = x_test.flatten(start_dim=1)
    for i in range(num_train):
        dists[i] = torch.sqrt(torch.sum(torch.square(test-x_train[i].flatten()),dim = 1))
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # train = x_train.view(num_train,-1)
    # test = x_test.view(num_test,-1)
    # train = train.view(train.shape[0],1,-1)
    # dists = torch.sqrt(torch.sum(torch.square(train-test),dim=2))
    #citation:https://stackoverflow.com/questions/73372554/calculating-euclidean-distance-using-pytorch-with-no-loops-only-using-vectoriza/73375125#73375125
    x = x_train.flatten(start_dim =1)
    y = x_test.flatten(start_dim =1)
    x2 = x**2
    y2 = y**2
    # print(x2.shape)
    # print(y2.shape)
    x2 = torch.sum(x2, dim =1, keepdim = True)
    y2 = torch.sum(y2, dim =1, keepdim = True)
    # print(x2.shape)
    # print(y2.shape)
    xy2 = torch.mm(x,y.t())
    dists = torch.sqrt(x2 - 2. * xy2 + y2.t())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    val,ind = torch.topk(dists,k,largest = False, dim =0)
    y = y_train[ind]
    y_pred,_ = torch.mode(y,dim = 0)
...
def knn_cross_validate(
    ...
    # Replace "pass" statement with your code
    for k in k_choices :
        x_train_sgd = torch.tensor([])
        y_train_sgd = torch.tensor([])
        k_acc = []
        for fold in range(num_folds) :
            x_valid = x_train_folds[fold]
            y_valid = y_train_folds[fold]
            for cat in range(num_folds):
                if cat!=fold :
                    if (torch.numel(x_train_sgd)==0):
                        x_train_sgd = x_train_folds[cat]
                        y_train_sgd = y_train_folds[cat]
                    x_train_sgd = torch.cat((x_train_sgd,x_train_folds[cat]),0)
                    y_train_sgd = torch.cat((y_train_sgd,y_train_folds[cat]),0)
            
            classifier = KnnClassifier(x_train_sgd, y_train_sgd)
            k_acc.append(classifier.check_accuracy(x_valid, y_valid, k=k, quiet=True))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    mean_acc = {}
    for k,val in k_to_accuracies.items() :
