...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    scores = X.mm(W)
    lst = torch.arange(num_train)
    correct_class_score = scores[lst,y]
    margins = torch.clamp(scores - correct_class_score.unsqueeze(1) + 1, min=0)
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    ind = torch.randint(low = 0,high = num_train,size=(batch_size,))
    X_batch = X[ind]
    y_batch = y[ind]
...
def svm_get_search_params():
    ...
    regularization_strengths = [1e0,1.1e0,1e1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    # num_iters = 100
    _ = cls.train(data_dict['X_train'],data_dict['y_train'],learning_rate=lr, reg=reg, num_iters=num_iters, batch_size=150, verbose=False)
    
    y_pred = predict_linear_classifier(cls.W, data_dict['X_train'])
    train_acc = 100.0 * (data_dict['y_train'] == y_pred).double().mean().item()
    
    val_pred = predict_linear_classifier(cls.W, data_dict['X_val'])
    val_acc = 100.0 * (data_dict['y_val'] == val_pred).double().mean().item()
    # Replace "pass" statement with your code
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        score_i = X[i]@(W) #score_i will have the scores for one training example and will have a shape 1 x C
        #shifting for numerical stability 
        score_i -= torch.max(score_i) #making the largest value 0 by subtracting max 
        # print('score_i =',score_i)
        prob_score_i = torch.exp(score_i)/torch.sum(torch.exp(score_i)) 
        # print('prob_score = ',(prob_score_i))
        # print(y[i])
        loss += -(torch.log(prob_score_i[y[i]])) #y(i) gives the true class / negative log of the G.T class is the loss component
        # print('prob = ',torch.log(prob_score_i))
        for j in range(num_class):
            if j==y[i]:
                dW[:, j] += (prob_score_i[j] - 1) * X[i]
            else:
                dW[:, j] += prob_score_i[j] * X[i] 
    # print('loss = 'loss)    
    loss /= num_train  #dividing by the number of training examples 
    loss += reg * torch.sum(W * W)
    dW /= num_train
    dW += 2 * reg * W 
    #############################################################################
...
def softmax_loss_vectorized(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    #citation : chatgpt concept from svm vectorized was used 
    num_train = X.shape[0]
    num_class = W.shape[1]
    
    scores = torch.mm(X,W)  # shape = N x C
    max_scores =  torch.max(scores, dim =1 ).values # vector of N 
    max_scores = max_scores.view(-1,1)
    scores = scores - max_scores 
    exp_scores = torch.exp(scores) #shape = N x C
    sum_classes = torch.sum(exp_scores, dim = 1) # shape of N 
    sum_classes = sum_classes.view(-1,1)
    exp_scores = exp_scores / sum_classes #shape = N x C
    # print('exp = ',exp_scores)
    loss = -torch.sum(torch.log(exp_scores[torch.arange(num_train),y])) #scalar value
    # print('loss=', loss)
    loss = loss / num_train
    loss += reg * torch.sum(W * W)
    exp_scores[torch.arange(num_train), y] -= 1
    dW = torch.mm(X.t(), exp_scores)
    dW = dW / num_train
    dW += reg * W
...
def softmax_get_search_params():
    ...
    regularization_strengths = [1e-3, 1e-1, 1e1, 1e3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
